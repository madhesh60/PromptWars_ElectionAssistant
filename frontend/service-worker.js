    ...
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then(cache => {
                ...
                return cache.addAll(ASSETS_TO_CACHE).catch(err => console.warn('Cache error (ignored for demo):', err));
            })
    );
});
...
self.addEventListener('push', event => {
    let data = { title: "Election Alert", body: "An important date is approaching." };
    if (event.data) {
